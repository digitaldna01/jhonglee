...
| **부모-자식(계층) 청킹** | 작은 자식 청크(128–256tok)로 검색, 큰 부모(512–1024tok)를 LLM에 제공 | 2025–26 프로덕션에서 가장 널리 채택된 패턴 | ✅ **채택** — 섹션 청크로 검색; 컨텍스트는 1위 문서 **본문 전체**(최대 포스트 ~11KB ≈ 3k tok, 2026-09-07) + 2–4위 summary+해당 섹션. A/B: 발췌만으로는 "영상 어떻게 만들었어?"에 오답, 전문으로는 정답(§2.12) |
...
| **리랭킹 (크로스인코더)** | 후보를 질문과 함께 재채점 | 정밀도 대폭↑, 느려서 사전 필터된 집합에만 적용 | ✅ **채택 (2026-09-07)** — ms-marco-MiniLM-L-6 int8 (fastembed, `retrieval/reranker.py`). Pi 실측 +234MB·10쌍 ~1s. 영어 전용으로 충분(재작성이 영어화). 게이트 필수: 로짓 ≥0인 후보만 승격, 미달 시 hybrid 순서 유지 — 무게이트는 메타 질문("Who are you?")에서 EN 12→7 붕괴. 앵커 폴백 쿼리는 스킵(주제어가 없어 노이즈 채점). 전체 파이프라인 EN 12/12 · KO r@1 14/14 |
...
**후속(2026-09-01) — 기계로 잡을 수 있는 건 판정관에게 묻지 않는다.** 판정은 실행마다 ±1 흔들려 기계적 규칙(합쇼체·이모지·느낌표 2개+)의
회귀가 노이즈에 묻힐 수 있다. `scripts/style_check.py`가 정규식으로 정확히 검사하고("~니까"는 연결어미와 구분: 앞 음절 ㅂ받침일 때만 합쇼체)
`judge_answers`가 모든 답에 적용해 리포트에 합산한다. 용어 오역(statevector→"통계벡터", 실서비스 로그)은 단어 열거 대신 일반 규칙로 프롬프트에
추가: 전문용어는 문서 표기 그대로, 표준 번역어가 확실할 때만 한국어로, 번역어를 지어내지 않기. 잘림은 `usage_report`가
`output_tokens == CHAT_MAX_TOKENS`인 답을 세는 것으로 감시(0이 아니면 길이 규칙 의심).
...
## 2.12 리랭커 + 1위 문서 전문 + 1인칭 우선 (2026-09-07)

발단은 프로덕션 로그: "폰트 작업은 한게 없어?"에 검색이 Visual Art Portfolio를 1위, Gill Sans를 4위로 놓았고(답변 모델이
구제), "Gill Sans 더 자세히"의 답이 서체 백과사전 발췌처럼 읽혔다(본문 6청크 중 5개가 서체 역사, "내가 만든" 얘기는 141자).
셋을 한 묶음으로:

- **크로스인코더 리랭커** (`retrieval/reranker.py`, ms-marco-MiniLM-L-6-v2 int8): hybrid 상위 10개 문서의 인용 청크를
  재채점해 문서 순서 결정. **게이트 ≥0 필수** — 프로토타입은 3개 실패 케이스를 전부 1위로 고쳤지만, 무게이트 골든셋에선
  EN 12/12→7/12 붕괴("Who are you?" 같은 메타 질문은 모든 쌍이 저점인데 그 노이즈로 재정렬함). 게이트를 넘는 후보만
  승격하고 나머지는 hybrid 순서 유지 → 회귀 0. **앵커 폴백 쿼리는 스킵** — "Tell me more about it"엔 주제어가 없어
  크로스인코더가 채점할 근거가 없고, 테스트가 실제로 앵커 무력화를 잡아냄. 영어 전용 모델로 충분한 건 재작성(§2.10)이
  한국어를 먼저 영어화하기 때문이며 한글 쿼리는 스킵. Pi 실측: +234MB RSS(한도 1300m 내), 로드 16s(부팅 1회).
  `RERANK_MODEL=""`로 끔. Dockerfile이 임베딩 모델처럼 이미지에 굽는다.
  **지문 캡 320자 필수** (배포 직후 발견): 어텐션 비용이 길이에 ~제곱이라 실전 길이(평균 1,242자) 10쌍이 Pi에서
  14s — 프로덕션 retrieval_ms 4–7s로 관측됨. 320자 캡으로 0.86s, 골든셋 동일(제목·헤딩·첫 문장이 주제를 다
  담고, post-writing-guide가 첫 문장에 답을 두게 함). 짧은 지문으로 잰 사전 벤치가 함정이었다 — 실측은 실물로.
- **1위 문서 전문** (`prompts.build_context`): 최대 포스트가 ~11KB ≈ 3k tok라 부모-문서 검색을 온전히 — 1위는 전 섹션
  (제목 유지), 2–4위는 summary+발췌 유지(주연 희석 방지). A/B(리랭커 동일): "그 영상 어떻게 만든 거예요?"에 발췌만은
  "영상이 아닌데"라고 오답, 전문은 스톱모션 제작 과정을 정답. 질문당 +~3k 입력 토큰 ≈ +$0.003 (Haiku).
- **1인칭 우선 프롬프트 한 줄**: "You are the maker, not an encyclopedia — lead with what you made." 전문이 준
  making-of 재료와 결합해 "I printed them, cut each one out by hand"로 답이 시작됨.

전체 파이프라인(hybrid + rewrite + rerank) 골든셋: EN 12/12 · KO r@1 14/14 · FU-A 11/11 · B 5/5 · D 2/2 — 역대 최고.
남은 미스는 재작성 출력 변동(§2.10) 계열뿐. 교훈: 리랭커는 "어느 문서가 1위냐"를, 전문은 "1위에서 무엇을 말할 수 있냐"를
고친다 — 대체재가 아니라 보완재.
