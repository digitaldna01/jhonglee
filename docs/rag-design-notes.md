...
- 그래프 엣지도 같은 임베딩에서 파생 — 전체 쌍 코사인의 평균 + 0.5σ 이상인 쌍만, 나머지는 부유 (§2.8)
...
## 2.8 랜딩 그래프 엣지 — 상위 k개에서 z-score 임계값으로 (2026-08-29)
...
**모양과 배치 (프런트, 같은 날)**
- 물리는 Obsidian 그래프 뷰 참고: 부드러운 원형 경계(`RADIUS`·`RADIAL`)로 사각형 대신 원반, 낮은 마찰(`DAMP`)로 느린 정착,
  부유 노드는 중심 인력 ×3(`FLOAT_GRAVITY`)으로 가장자리 대신 덩어리 사이 빈자리에. 드래그 후 튕김은 `SPRING`·`RADIAL`·`DAMP`를 한 단계씩 낮춰 해결
